...
// SHUNYA: License check - No license, no AI!
      if (!hasActiveLicense) {
        setState((prev) => ({
          ...prev,
          error: "Shunya Pro License is required. Please activate it in settings.",
        }));
        return;
      }
  ...
  useEffect(() => {
    ...
    const handleStorageChange = async (e: StorageEvent) => {
      if (e.key === "Shunya-conversation-selected" && e.newValue) {
        try {
      ...
      try {
        if (prompt) {
          ...
          setState((prev) => ({
            ...prev,
            attachedFiles: [...prev.attachedFiles, attachedFile],
            input: "📷 Analyze this image and provide a brief.", // SHUNYA: Auto-fill prompt for quick results
          }));
