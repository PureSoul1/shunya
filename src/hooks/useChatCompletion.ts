...
import { MAX_FILES, AUTO_SUGGESTION_PROMPT } from "@/config";
...
export const useChatCompletion = (
  ...
  const submit = useCallback(
    async (speechText?: string) => {
      ...
      // SHUNYA: License check - No license, no AI!
      if (!hasActiveLicense) {
        setState((prev) => ({
          ...prev,
          error: "Shunya Pro License is required. Please activate your license in settings.",
        }));
        return;
      }
    ...
    async (meetingContext: string) => {
      if (!meetingContext.trim()) return "";

      // SHUNYA: Pro feature check
      ...
      try {
        ...
        // AI ko silently request bhej rahe hain, UI mein stream nahi karenge
        for await (const chunk of fetchAIResponse({
          provider: useShunyaAPI ? undefined : provider,
          selectedProvider: selectedAIProvider,
          systemPrompt: AUTO_SUGGESTION_PROMPT, // Humara special suggestion prompt
          history: [], // History ki zaroorat nahi, sirf current context kaafi hai
    ...
    async (base64: string, prompt?: string) => {
      ...
      try {
        if (prompt) {
          ...
          // Manual mode: Add to attached files AND auto-fill prompt
          const attachedFile: AttachedFile = {
            id: Date.now().toString(),
            name: `screenshot_${Date.now()}.png`,
            type: "image/png",
          ...
          setState((prev) => ({
            ...prev,
            attachedFiles: [...prev.attachedFiles, attachedFile],
            input: "📷 Analyze this image and provide a brief.", // SHUNYA: Auto-fill prompt
          }));

          // Auto-focus input so user just has to hit Enter
          setTimeout(() => {
            inputRef.current?.focus();
          }, 100);
        }
        ...
        if (!hasPermission) {
          ...
          if (!hasPermissionNow) {
            setState((prev) => ({
              ...prev,
              error:
                "Screen Recording permission required. Please enable it by going to System Settings > Privacy & Security > Screen & System Audio Recording. If you don't see Shunya in the list, click the '+' button to add it. If it's already listed, make sure it's enabled. Then restart the app.",
            }));
