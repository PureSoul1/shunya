...
export function useSystemAudio() {
  ...
  // SHUNYA UPGRADE: Live Transcript Array for continuous context
  ...
  // SHUNYA UPGRADE: Store transcript instead of calling AI immediately
  useEffect(() => {
          ...
          try {
            ...
            try {
              ...
              if (transcription.trim()) {
                ...
                // SHUNYA UPGRADE: Save to live transcript instead of calling AI immediately
  ...
  // SHUNYA UPGRADE: Use liveTranscript for Quick Actions to provide instant context
  const handleQuickActionClick = async (action: string) => {
    ...
    // Combine the last 5 transcript chunks for context
    ...
    // Clear the live transcript after AI processes it to avoid repetition
    setLiveTranscript([]);
  };
  ...
  const startCapture = useCallback(async () => {
    try {
      ...
      setRecordingProgress(0);
      setLiveTranscript([]); // SHUNYA UPGRADE: Clear transcript on new capture
  ...
  const stopCapture = useCallback(async () => {
    try {
      ...
      setLiveTranscript([]); // SHUNYA UPGRADE: Clear transcript on stop
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : String(err);
  ...
  const startNewConversation = useCallback(() => {
    ...
    setLiveTranscript([]); // SHUNYA UPGRADE: Clear transcript on new conversation
  }, []);
