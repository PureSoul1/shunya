...
export const Sidebar = () => {
  ...
  return (
    <aside className="flex w-56 flex-col h-screen select-none pt-2">
      {/* Logo */}
      <div
        onClick={() => navigate("/dashboard")}
        className="flex h-16 items-center px-4 pt-10 gap-1.5 cursor-pointer"
      >
        {/* SHUNYA: Orange 'S' Icon */}
        <div className="flex size-6 lg:size-7 items-center justify-center rounded-md bg-orange-600 text-white font-bold text-xs lg:text-sm shadow-md">
          S
        </div>
      ...
      {/* Footer Section */}
      <div className="mt-auto flex flex-col space-y-1 px-3 pb-3">
        ...
        {/* SHUNYA: Agentic Fox Labs Branding */}
        <div className="mt-2 pt-2 border-t border-sidebar-border/50">
          <p className="text-[9px] text-muted-foreground/60 text-center mb-1">
            Developed by
          </p>
          <button
            onClick={() => openUrl("https://agenticfoxlabs.com")}
            className="flex items-center justify-center gap-1.5 w-full text-xs text-muted-foreground hover:text-orange-500 transition-colors py-1 cursor-pointer"
          >
            <span className="font-bold tracking-wide">Agentic Fox Labs</span>
