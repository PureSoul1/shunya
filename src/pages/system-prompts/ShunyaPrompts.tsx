...
interface ShunyaPromptsResponse {
  prompts: ShunyaPrompt[];
  total: number;
  last_updated?: string;
}
...
const SELECTED_Shunya_PROMPT_STORAGE_KEY = "selected_Shunya_prompt";

export const ShunyaPrompts = () => {
  const {
    setSystemPrompt,
    hasActiveLicense,
    setSupportsImages,
  ...
  const [prompts, setPrompts] = useState<ShunyaPrompt[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [lastUpdated, setLastUpdated] = useState<string | null>(null);
  const [selectedShunyaPrompt, setSelectedShunyaPrompt] =
    useState<ShunyaPrompt | null>(() => {
      // Load selected prompt from local storage on initial render
      const stored = safeLocalStorage.getItem(
        SELECTED_Shunya_PROMPT_STORAGE_KEY
      );
      if (stored) {
        try {
  ...
  // Watch for changes in user's selected prompt and clear Shunya selection if needed
  useEffect(() => {
    const checkUserPromptSelection = () => {
      const userSelectedPromptId = safeLocalStorage.getItem(
        STORAGE_KEYS.SELECTED_SYSTEM_PROMPT_ID
      );
      // If user has selected one of their own prompts, clear Shunya prompt selection
      if (userSelectedPromptId) {
        setSelectedShunyaPrompt(null);
      }
    };
  ...
  const fetchShunyaPrompts = async () => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await invoke<ShunyaPromptsResponse>("fetch_prompts");
      setPrompts(response.prompts);
      if (response.last_updated) {
        setLastUpdated(response.last_updated);
      }
    } catch (err) {
      console.error("Failed to fetch Shunya prompts:", err);
      setError(
        typeof err === "string" ? err : "Failed to fetch Shunya prompts"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSelectShunyaPrompt = async (prompt: ShunyaPrompt) => {
    ...
    try {
      ...
      setSelectedShunyaPrompt(prompt);
      ...
      // Save the selected Shunya prompt to local storage for persistence
      ...
      console.error("Failed to select Shunya prompt:", error);
    }
  };

  const handleCardClick = (prompt: ShunyaPrompt) => {
    handleSelectShunyaPrompt(prompt);
  };

  const isPromptSelected = (prompt: ShunyaPrompt) => {
    return (
      selectedShunyaPrompt?.title === prompt.title &&
      selectedShunyaPrompt?.modelId === prompt.modelId
    );
  };
    ...
    return (
      <div className="space-y-4 mt-6">
        ...
        <Empty
          ...
          description="Fetching Shunya default prompts"
        />
      </div>
    );
  }
