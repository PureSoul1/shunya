// Shunya AI Speech Detection, and capture system audio (speaker output) as a stream of f32 samples.
...
pub async fn start_system_audio_capture(
    ...
    // SHUNYA FIX: If capture is already running, safely abort the old one before starting fresh
    {
        let mut guard = state
            .stream_task
            .lock()
        ...
        if let Some(old_task) = guard.take() {
            warn!("Capture already running, aborting previous task before restarting");
            old_task.abort(); // Kill the old capture
            
            // Reset capturing state
            if let Ok(mut cap_guard) = state.is_capturing.lock() {
                *cap_guard = false;
            }
        }
    }
