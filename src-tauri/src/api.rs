...
struct SecureStorage {
    ...
    selected_shunya_model: Option<String>,
}
...
#[derive(Debug, Serialize, Deserialize)]
pub struct ShunyaPromptsResponse {
    prompts: Vec<ShunyaPrompt>,
    total: i32,
...
// Fetch Shunya Prompts API
#[tauri::command]
pub async fn fetch_prompts() -> Result<ShunyaPromptsResponse, String> {
    let app_endpoint = get_app_endpoint()?;
    let api_access_key = get_api_access_key()?;
    ...
    let prompts_response: ShunyaPromptsResponse = response
        .json()
        .await
